...
- **Explicit first, clean as a fallback** (the user's rule).
  ...
  - On `ytdlp.ErrAgeRestricted` or `ErrUnplayable`, `downloader.tryOtherUploads` tries `youtube.Alternatives`, then `Resolver.MoreAlternatives` (a wider search), then `itunes.CleanEdit`. A stand-in must be the same recording: no variant words, and the full title minus "feat.", because explicit and clean often differ only inside the brackets ("Lovin'" / "Fuckin'").
