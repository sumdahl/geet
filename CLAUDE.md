...
- `cmd/geet/trending.go`, `internal/deezer/chart.go`, `internal/itunes/top.go`: `geet trending`.
  - Deezer's `/chart/0/tracks` is geo-localised by the request's own address (from Nepal it returns Nepali songs), which is why it leads; Apple's per-country feed at `rss.marketingtools.apple.com` tops it up. **The old `itunes.apple.com/<cc>/rss/topsongs` feed returns years-old entries — don't use it.**
  - Output is `searchResult` plus `rank`, so the plugin reuses its search row. Cached in `~/.cache/geet/trending-<source>[-<country>].json` for `trending.cache_for`; a failed fetch falls back to a chart up to a week old rather than showing nothing.
  - A chart entry names only the main artist and has no year or ISRC: `fullTrack` reads the full Deezer entry once a song is actually picked.
- `internal/player`, `internal/player/spectrum`, `internal/player/tui`, `internal/lyrics`, `cmd/geet/play.go`: `geet play`, the terminal player.
  - **Streaming first** (`internal/player/stream.go`): a song that isn't downloaded is matched with the usual `youtube.Resolver` and played from the URL `yt-dlp -g -f bestaudio/best` prints; mpv and ffmpeg both read it, so the spectrum works on a stream too. Streams are resolved as a song comes up (plus one prefetched), never for a whole queue at once. `d` runs the ordinary download pipeline for the playing track and swaps in the file when it lands. `player.stream = false` goes back to downloading first.
  ...
  - **Complex scripts** (`internal/player/tui/width.go`): a terminal may draw Devanagari and its relatives at a width Unicode doesn't predict, so `Width` measures them at their worst case (the rune count), `Truncate` cuts only between grapheme clusters, and `body` gives such lyrics the full width with the spectrum above instead of a column beside them. Latin text keeps the two-column layout.
