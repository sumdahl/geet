...
- Thin Quickshell-side wrapper that spawns `geet watch --json` (or
...
- Example Hyprland keybind for people who don't want the daemon running:
  `SUPER+SHIFT+Y` → `geet download "$(wl-paste)" --json`.
- Repo: `omarchy-plugin-geet` (see `00-overview.md` for naming/paths).
